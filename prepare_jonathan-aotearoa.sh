...
if [ ! -f target/CalculateAverage_jonathan-aotearoa_image ]; then
    ...
    JAVA_OPTS="--enable-preview -da -dsa"
    # Use the no-op GC.
    # Enable CPU features (-march=native) and level-3 optimisations (-O3)
    NATIVE_IMAGE_OPTS="--gc=epsilon -O3 -march=native"
    NATIVE_IMAGE_OPTS="$NATIVE_IMAGE_OPTS --initialize-at-build-time=dev.morling.onebrc.CalculateAverage_jonathanaotearoa $JAVA_OPTS"
