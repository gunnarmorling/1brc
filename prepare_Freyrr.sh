...
if [ ! -f target/CalculateAverage_Freyrr_image ]; then
    ...
    NATIVE_IMAGE_OPTS="$NATIVE_IMAGE_OPTS --enable-preview --initialize-at-build-time=dev.morling.onebrc.CalculateAverage_Freyrr\$Scanner"
    ...
    # NATIVE_IMAGE_OPTS="$NATIVE_IMAGE_OPTS -H:MethodFilter=CalculateAverage_Freyrr.* -H:Dump=:2 -H:PrintGraph=Network"
    
    native-image $NATIVE_IMAGE_OPTS -cp target/average-1.0.0-SNAPSHOT.jar -o target/CalculateAverage_Freyrr_image dev.morling.onebrc.CalculateAverage_Freyrr
fi
